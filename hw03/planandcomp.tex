\begin{homeworkProblem}{Planarity and Graph Complements}
    ...
    $\overline{G}$ must be non-planar. After 13 we continue to have more than the
    $3v-6$ edges allowed in a planar graph, so the complement must not be planar.
    \\ \\
    \part
    Consider a graph $G$ with $K_5$ but all other vertices unconnected. The
    complement must then also then be non planar, as some other group of 5 
    vertices in the 8 unconnected ones are now connected, also forming $K_5$.
\end{homeworkProblem}