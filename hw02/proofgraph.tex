\begin{homeworkProblem}{Proofs in Graphs}
    ...
    \begin{itemize}
        ...
        \item \textit{Inductive step}: Now, consider $k=n+1$. Remove a city $P$ from the road configuration, leaving a road configuration with only $n$ cities. By the inductive hypothesis, there exists a city $B$ in this smaller road configuration for which the claim holds. Now there are two cases:

        \begin{itemize}
            \item Case 1: $P$ has a road to $B$. Then, it is reachable by traveling through one road. Since all cities can reach $B$ in at most two roads the claim holds.
            \item Case 2: $B$ has a road to $P$. Then, $B$ is in the set of cities with a road to $P$. Since, from the inductive hypothesis, every city with a road from $P$ has a road to a city with a road to $P$, they can reach $P$ in two steps. $P$ is the city which is reachable from every other city. 
        \end{itemize}

        Thus, by the principle of induction we have shown that the claim holds regardless of the configuartion of roads. 
    \end{itemize}
