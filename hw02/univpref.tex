\begin{homeworkProblem}{Universal Preference}
    ...
    We get the same pairing from running the algorithm with candidates proposing. Since the preferences are universal and mirrored from jobs to candidates, the candidates will propose and the jobs will reject in the exact same way as when the jobs proposed and the candidates rejected.
    \\ \\
    \part
    
    Because both the job-optimal and candidate-optimal stable matching algorithms produce the same pairing, there is only 1 stable pairing, and no intermediate stable pairings exist.
